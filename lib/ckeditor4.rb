...
module Ckeditor4
  autoload :Utils, 'ckeditor4/utils'
  autoload :Http, 'ckeditor4/http'
  autoload :TextArea, 'ckeditor4/text_area'
  autoload :Paginatable, 'ckeditor4/paginatable'

  module Helpers
    autoload :ViewHelper, 'ckeditor4/helpers/view_helper'
    autoload :FormHelper, 'ckeditor4/helpers/form_helper'
    autoload :FormBuilder, 'ckeditor4/helpers/form_builder'
    autoload :Controllers, 'ckeditor4/helpers/controllers'
  end

  module Hooks
    autoload :SimpleFormBuilder, 'ckeditor4/hooks/simple_form'
    autoload :CanCanAuthorization, 'ckeditor4/hooks/cancan'
    autoload :PunditAuthorization, 'ckeditor4/hooks/pundit'
  end

  module Backend
    autoload :Paperclip, 'ckeditor4/backend/paperclip'
    autoload :CarrierWave, 'ckeditor4/backend/carrierwave'
    autoload :Dragonfly, 'ckeditor4/backend/dragonfly'
  end
  ...
  # Default way to setup Ckeditor4. Run rails generate ckeditor to create
  ...
  def self.base_path
    @base_path ||= (asset_path || File.join([Rails.application.config.assets.prefix, '/ckeditor4/']))
  end

  # All css and js files from ckeditor folder
  def self.assets
    @@assets ||= Utils.select_assets("ckeditor4", "vendor/assets/javascripts") << "ckeditor4/init.js"
  end
    ...
    if block_given?
      ...
      @@picture_model_class ||= begin
        if @@picture_model.respond_to? :call
          ...
          @@picture_model || Ckeditor4::Picture
        end
      end
    end
  end
    ...
    if block_given?
      ...
      @@attachment_file_model_class ||= begin
        if @@attachment_file_model.respond_to? :call
          ...
          @@attachment_file_model || Ckeditor4::AttachmentFile
        end
      end
    end
  end
  ...
  def self.authorize_with(*args, &block)
    ...
    if extension
      @authorize = Proc.new {
        @authorization_adapter = Ckeditor4::AUTHORIZATION_ADAPTERS[extension].new(*([self] + args).compact)
      }
    else
...
require 'ckeditor4/engine'
require 'ckeditor4/version'
